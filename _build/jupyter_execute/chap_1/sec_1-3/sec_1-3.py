...
# 
# これを回避するには以下の方法がある：
# 1. ［推奨］`r"C:\Users\username\OneDrive\情報処理の応用"`とする（raw文字列）
...
# ここでは，Pythonスクリプトファイルのパス（保存先）が`"C:\Users\username\OneDrive\情報処理の応用\chap_1\test.py"`であるとする．
# これを実行するには以下の方法がある：
# 
...
get_ipython().run_line_magic('run', '"C:\\Users\\parar\\OneDrive\\情報処理の応用\\chap_1\\test.py"')
...
os.chdir(r"C:\Users\parar\OneDrive\情報処理の応用") # カレントディレクトリの移動
get_ipython().run_line_magic('run', '"./chap_1/test.py"             # 実行')


# 2つ目の方法では，まず`os.chdir`コマンドで`C:/Users/username/OneDrive/情報処理の応用`という作業フォルダに移動してから，さらに"./2_environment/test.py"を実行している．
# このとき，現在の居場所（今の場合は[情報処理の応用]フォルダ）のことをカレントディレクトリと呼ぶ．
...
import os
os.chdir(r"C:\Users\parar\OneDrive\情報処理の応用")
...
fig.savefig('./chap_1/graph.pdf', bbox_inches="tight", pad_inches=0.2, transparent=True, dpi=300)
