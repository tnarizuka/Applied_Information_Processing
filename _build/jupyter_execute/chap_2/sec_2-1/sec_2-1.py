...
# また，これらは，データにどのように数値を対応させるかという規則（**尺度**）によってさらに細かく分類できる．

# ### 質的データ

# まず，性別（男，女），学歴（大卒，高卒，中卒），天気（晴，曇，雨，雪）など，他と区別することだけができるデータを**カテゴリカルデータ**と呼ぶ．
# カテゴリカルデータは，「男を１，女を２」とすれば数値で表すことができるが，その数値には他と区別するという以上の意味はない．
# このような数値の割り当て方（尺度）は**名義尺度**と呼ばれる．
...
# このような数値の割り当て方は（尺度）は**順序尺度**と呼ばれる．
...
# ```
# なお，間隔データは値同士の加減が許される．
# 
# 最後に，値の大小関係と値の差に加えて，値同士の比にも意味があるようなデータを比率（比例）データと呼ぶ．
...
# 例えば，摂氏温度や時刻における0は便宜的に定めたものであるが，身長や体重が0であるときは本当に無いので，前者は間隔尺度，後者は比率尺度の例である．
