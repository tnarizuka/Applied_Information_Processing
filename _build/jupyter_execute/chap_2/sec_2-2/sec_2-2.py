...
# 同様にして，階級$ i $の平均所得を$ y_{i} $，平均所得の累積割合を$ Y_{i} $とする．
# この場合，$ Y_{i} $は下から$ i $までの階級の平均所得が全体に占める割合となる．
# 
# 例えば，100人の集団を4つの階級（$n=4$）に分けた場合は以下のようになる（所得は適当に設定している）．
...
pd.DataFrame(np.c_[x, X, y, Y],
             columns=['度数xi', '累積相対度数Xi', '平均所得yi', '所得の累積割合Yi'],
             index=[1,2,3,4])
