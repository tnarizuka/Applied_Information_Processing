...
# 変数$ X,\ Y $の散布図に右上がりや右下がりの傾向がある場合には相関関係があるという．
...
# $$(eq:covariance)
# 
# ただし，2つ目の等式では分散公式{eq}`eq:deviation2`を使った．
# ```{margin}
# $ 共分散は\mathrm{Cov}(X, Y) $と表すこともある．
# ```
# 共分散の意味は以下の通りである．
# まず，散布図の中心として，平均値$ (\bar{x}, \bar{y}) $を考える．
# その上で，データ$ (x_{i}, y_{i}) $に対し，平均値から$ X,\ Y $方向への増分（つまり偏差）の積$ (x_{i}-\bar{x})(y_{i}-\bar{y}) $を計算している．
# この量は，データ$ (x_{i}, y_{i}) $が平均値$ (\bar{x}, \bar{y}) $より右上にまたは左下にあれば正，左上または右下にあれば負となる．
# よって，$ (x_{i}-\bar{x})(y_{i}-\bar{y}) $を全データに対して平均した量（共分散）を考えれば，全体的な右上がり，右下がりの傾向を定量化できる．

# ### 相関係数

# 共分散は$ X,\ Y $の単位やばらつきの度合いによって値が大きく異なってしまい，相関の程度を一定の基準で表すことができない．
...
# 相関係数は，共分散と異なり変数$X,\ Y$の単位やばらつきの度合いに依らない量である．
# 
# なお，分散公式{eq}`eq:deviation2`を用いると，相関係数は以下のように表される：
# 
# $$
# 	r_{XY} = \frac{\overline{xy} - \bar{x}\bar{y}}{(\overline{x^{2}} - \bar{x}^{2})(\overline{y^{2}} - \bar{y}^{2})}
# $$(eq:correlation2)

# 相関係数の定義域は$ -1\leq r_{XY} \leq 1 $であり，その絶対値によって相関の強さを表すことができる．
