...
# - 1回のベルヌーイ試行において，成功，失敗を1と0に対応させた確率変数 $ X $ はベルヌーイ分布に従う．
...
# 
# - ベルヌーイ分布に従う確率変数の和
# - 具体例：ベルヌーイ試行を $ n $ 回繰り返すときの成功回数 $ X $（コイン投げを繰り返した時に表の出る回数）は二項分布に従う．
...
# 
# - 二項分布において平均を $ np = \lambda $ とおき，$ \lambda $ を一定に保ったまま $ n\to \infty,\ p\to 0 $ とした場合．
# - 稀な現象（ $ p\to 0 $ ）を大量に（ $ n\to \infty $ ）観測したとき，一定期間内の成功数が従う確率分布．
...
# $$ f(x) = (1-p)^{x-1}p $$
# 
# - ベルヌーイ試行を繰り返すとき，初めて成功した時点で失敗した回数 $ X $ は幾何分布に従う．
...
# 
# - 中心極限定理「任意の確率分布に従う $ n $ 個の確率変数の和は $ n $ を大きくしたときに正規分布に従う」から導かれる．
# - 二項分布で $ n\to \infty $ とした場合は中心極限定理の一例．
# - 具体例：様々な自然現象，社会現象において観られる（成人の身長，測定誤差，など）
...
# 
# - 連続的な時間で事象が独立に一定の確率で生じるような確率過程を**ポアソン過程**と呼ぶ．
# - ポアソン過程において，２つの連続する事象間の待ち時間分布は指数分布に従う．
# - 幾何分布の連続版．
...
# - ランダム乗算過程では，$ t\to \infty $ において $ X_{t} $ が対数正規分布に従う．
...
# - べき分布が現れる背景には要素間の複雑な相互作用が存在することが多く，その出現メカニズムは未だに研究対象となっている{cite}`松下2019,Buchanan2009`．
